...
$(function(){
		...
		$('.loader').css('display','none');
	 	...
	 	$(document).on("click", ".start-btn", function(){
	 		...
	 		var brandUrl = $("input[name=brand]").val();
			var pageSize = $("input[name=pageSize]").val();
			var page = $("input[name=page]").val(); 
			var writer_name = $("input[name=writer_name]:checked").val();
	
		 	ajaxCallGet("/api/v1/brandstore?brandUrl="+brandUrl+"&page="+page+"&pageSize="+pageSize+"&writer_name="+writer_name, function(res){
				if(!res.success){
				 	alert(res.errorMsg) 
				}else{
					$('.loader').css('display','none');
		 ...
		 $(document).on("click", ".save-btn", function(){
				...
				var writer_name = $("input[name=writer_name]:checked").val();
		 	 	...
		 	 	ajaxCallGet("/api/v1/brand/save?brandUrl="+brandUrl+"&page="+page+"&pageSize="+pageSize+"&writer_name="+writer_name, function(res){
					if(res.success){
						$('.loader').css('display','none');
	                    $(".data-len").text(res.record);
	                   alert("저장되었습니다. ");
          ...
          <div class="content-wrapper">
     		  ...
     		  <div class="card">
                  <div class="card-body">
                    ...
                    <h4 class="card-title">브랜드 스토어</h4>
         ...
         <div class="modal fade" id="content_guide" tabindex="-1" role="dialog" aria-labelledby="#content_guide" aria-hidden="true">
                    ...
                    <form class="forms-sample">
                      <div class="form-group">
                        <label class="login-label" for="title">1. 브랜드 스토어 접속 > 상단 링크 복사 > 링크 입력란에 붙여넣기  </label>
                        <div class="img-container">
                        	<img style="width:100%" src="/resources/img/brand1.png"></div>
                      </div>
                  	 <div class="form-group">
                        <label class="login-label" for="title">2. [ 전체상품 ] 카테고리 기준 페이지  / 페이지당 가져올 개수 (최대 80개) </label>
                        <div class="img-container">
                        	<img style="width:100%" src="/resources/img/brand2.png"></div>
                      </div>
                      	 <div class="form-group">
